...
function semanticStyle(kind: string): NodeStyle | undefined {
  ...
  if (k === 'queue' || k === 'mq' || k === 'broker')
    return { backgroundColor: '#fff3bf', strokeColor: '#fab005' };
  ...
  if (k === 'decision')
    return { backgroundColor: '#ffd8a8', strokeColor: '#e8590c', strokeStyle: 'dashed' };
...
function mapNodeKind(kind: string | undefined): { type: NodeType; style?: NodeStyle } {
  ...
  if (k === 'diamond' || k === 'decision' || k === 'condition')
    return { type: 'diamond', style: semanticStyle(k) };
  if (
    k === 'ellipse' ||
    k === 'oval' ||
    k === 'user' ||
...
export function parseDSL(input: string): FlowchartGraph {
  ...
  for (const edge of directiveEdges) {
    const sourceNode =
      nodeRefs.get(edge.fromRef) || getOrCreateNode(edge.fromRef, 'rectangle', edge.fromRef);
    const targetNode =
      nodeRefs.get(edge.toRef) || getOrCreateNode(edge.toRef, 'rectangle', edge.toRef);
  ...
  for (const group of directiveGroups) {
    const nodeIds = Array.from(
      new Set(
        group.nodeRefs.map((ref) => nodeRefs.get(ref)?.id).filter((id): id is string => Boolean(id))
      )
    );
