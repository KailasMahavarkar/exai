 ...
 */
export async function shareExcalidraw(filePath: string, options?: ShareOptions): Promise<ShareResult> {
    // Read and validate
    const stat = statSync(filePath);
    if (stat.size > MAX_SIZE_BYTES) {
        console.warn(`Warning: File is ${(stat.size / 1024 / 1024).toFixed(1)}MB — large files may fail to upload.`);
    }
    ...
    if (options?.verbose) {
        console.log(`  Compressed: ${(compressed.length / 1024).toFixed(1)}KB (${((1 - compressed.length / stat.size) * 100).toFixed(0)}% reduction)`);
    }
