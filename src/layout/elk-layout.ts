...
// Dynamic ELK import
// Uses elkjs/lib/main.js which works with the web-worker package
...
async function loadELK() {
  ...
  try {
    const module = await import('elkjs/lib/main.js');
    ELK = module.default;
    return ELK;
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to load ELK layout engine: ${errorMsg}`);
  }
}
...
export async function layoutGraph(graph: FlowchartGraph, verbose: boolean = false): Promise<LayoutedGraph> {
  ...
  if (verbose) {
    console.log(`  Using ELK layout algorithm: layered`);
